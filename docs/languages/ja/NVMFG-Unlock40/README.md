...
[0.2.3 とステータスをダウンロード](../docs/downloads.md#nvmfg-unlock40) · [インストール](#installation) · [上流](#upstream-and-modifications) · [ライセンス](LICENSES/README.md)
...
現在のパッケージは **0.2.3** です。永続的なゲーム ライブラリ、アクティビティと機能の情報、ローカル診断、および修正された選択/進行動作が追加されます。 [ダウンロード](../docs/downloads.md#nvmfg-unlock40) は、正確なファイルとハッシュを識別します。
...
2. `NVMFGUnlock40-0.2.3-Setup-x64.exe` または `NVMFGUnlock40-0.2.3-Portable-x64.zip` のリリースが利用可能になったら、ダウンロードします。
...
**永続ライブラリ:** 1 回のスキャンを開始する前に、異なるドライブを含む複数のゲーム フォルダーを選択します。進捗状況が表示され、キャンセルも可能です。最初のスキャン後、すべてのゲーム フォルダーを移動することなく、起動時にローカル キャッシュによってライブラリが復元されます。更新して変更を見つけるか、別のフォルダーを追加します。メンテナンス操作では、引き続き影響を受けるファイルを再検証します。バックアップ監視はアクティブなままです。キャッシュは `%LOCALAPPDATA%\RtxMfg\library-cache.json` に保存されます。

**選択:** Ctrl+A はすべてを選択し、Ctrl+D はアクティブなゲームまたはバックアップ タブをクリアします。自動的に選択されるゲームはありません。アクティビティの更新と更新により、ゴースト選択や一貫性のないカウントが作成されなくなりました。

**アクティビティと互換性:** ゲームごとの MFG 情報は、新しいオーバーレイを使用しない NGX の観察に基づいています。表示されたフレームの物理的な数ではありません。 Dynamic と V-Sync のサポートは、ランタイム機能によって提供されます。不明な機能はバージョン番号から推測されません。アプリケーションは V-Sync も VRR も変更しません。 V-Sync がオフの場合、Dynamic はサスペンドされたままになります。固定またはゲーム制御の選択は別のものです。

**次回の起動:** 一時的な除外により、次回のゲーム起動時にパッチ適用がスキップされ、終了後に通常の管理が復元されます。ゲームに既にロードされている DLL は削除できません。ゲームを閉じて再起動してください。 Wallpaper Engine はデスクトップ アプリケーションとして認識されます。この修正により、実際に無視されたゲームの保護が維持されます。

**環境設定とサポート:** 環境設定のインポート/エクスポートには、ゲーム フォルダーを手動で再関連付けする必要があります。 「バージョン情報」のローカル診断では、個人情報がフィルタリングされ、利用可能な NVAPI エラー コードまたは競合カテゴリが報告されます。共有する前に確認してください。何も自動的にアップロードされません。

**アプリケーションの更新:** オプションのチェックを行うと、リリース ノートが表示され、公式セットアップが提供されます。明示的なダウンロードは、GitHub サイズおよび SHA-256 メタデータに対してチェックされます。インストールは自分で開始します。バージョン 0.2.3 では、意味のあるエラーと結果を保持しながら、完了した進行状況メッセージも消去されます。これらの追加には、パブリック バージョン 0.1.1 以降の変更が含まれています。
...
- 報告されている 0.1.1 のアクティブ化/復元/アンインストールのブロックは再現されておらず、その原因は不明です。このリリースではそれを修正するとは主張していません。障害が発生した後は、リカバリ ジャーナルを保存し、ローカル診断を検査します。リカバリデータを強制的に削除しないでください。

...
初回監査: 2026-09-09;現在のパッケージと来歴のレビュー: 2026-09-18。 NVDriverForge 0.1.4、NVMFG Unlock40 0.2.3、および NVRasterPulse 0.2 は、既存のコンポーネント ライセンスを保持します。 Profile Inspector 3.0.2.3/setup-r2 は変更されません。 **コンポーネントのライセンスは、スイート全体ではなく、そのコンポーネントをカバーします。** アプリケーションのソースは非公開のままです。著作権および許可通知はそのまま保持されます。このテーブルはインデックスであり、テーブルの代替ではありません。 「外部」とは、アプリケーション資産に分散されていないことを意味します。

...
| NVDriverForge | nvdriverforge-v0.1.4 | 既存の 0.x スキーム。バージョン管理された更新では以前のパッケージが保持されます |
| NVMFG Unlock40 | nvmfg-unlock40-v0.2.3 | アプリケーションのバージョン 0.2.3;パブリック 0.1.1 以降の累積的な変更 |
| NVRasterPulse | nvrasterpulse-v0.2 | 既存の 2 部構成バージョン |
...
9 月 18 日のアップデートでは、3 つの新しいタグが用意されています。既存の Profile Inspector リリースは変更されません。アセット名、タグ、および `SHA256SUMS.txt` は、アプリケーションの更新チェックのために正確なままでなければなりません。プレリリース フラグを付けずに通常のリリースを公開して、安定リリース チェックの対象にします。 NVMFG はまだ実験段階です。

<a id="integrity-and-storage"></a>
## 完全性と保管

パブリッシュされたバイナリ バイトをサイレントに置き換えないでください。新しいハッシュを含む新しい明示的なバージョンまたはインストーラー リビジョンを使用します。合法的なサイドカーは埋め込み通知を補足します。 NVDriverForge 0.1.4 ポータブルは 142,017,891 バイトで、GitHub の通常の 100 MiB Git ファイル制限を超えています。添付ファイルをリリースすると、このハブにバイナリまたは Git LFS を配置することがなくなります。 [GitHub 大きなファイルのガイダンス](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github)。

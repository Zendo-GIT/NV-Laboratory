...
[0.1.4 및 상태 다운로드](../docs/downloads.md#nvdriverforge) · [설치](#installation) · [크레딧](#credits-and-upstream) · [라이센스](../../../../NVDriverForge/LICENSE)
...
- 버전 0.1.4는 선택한 선택적 NVIDIA 구성 요소를 건너뛸 수 있도록 유지하고 확인되지 않은 확인되지 않은 구성 요소만 검색에서 제외합니다. 이미 최신이거나 적용할 수 없는 선택적 런타임은 더 이상 중요한 구성 요소로 강제되지 않습니다.
- 설치 실패 요약을 지우고 34개 언어로 된 자세한 로그에 액세스하세요.
- 설치 전 준비 상태 확인, 명시적 확인, 드라이버 저장소 내보내기 및 기본 NVIDIA 프로필 백업.
...
- 구성 요소 가이드, 재사용 가능한 기본 설정, 드라이버 키트, 로컬 지원 보고서 및 선택적 애플리케이션 업데이트.
...
**설치 전:** 준비 확인에는 패키지 서명, GPU, 예상 작업 공간/백업 공간, 보류 중인 다시 시작 및 경쟁 설치 프로그램이 포함됩니다. 고귀한 일꾼은 그것을 반복합니다. 경쟁하는 프로세스는 자동으로 중지되지 않습니다. NVIDIA 설치가 시작되기 전에 기본 NVIDIA 프로필-데이터베이스 백업이 성공해야 합니다. 드라이버 저장소 내보내기는 별도의 백업입니다.

**재사용 가능한 선택:** 구성 요소 가이드에서는 게임, 오디오, NVIDIA App 및 녹음에 대한 네 가지 질문을 묻습니다. 제안 사항을 검토하세요. 필수, 알 수 없음 및 종속성 구성 요소는 보호된 상태로 유지됩니다. 기본 설정을 내보낸 다음 가져올 때 선택한 패키지에 대해 미리 보고 유효성을 다시 검사합니다. 동의, 다시 시작 작업, 프로그램 경로 및 패치 페이로드는 가져오지 않습니다.

**드라이버 키트:** `.nvdfkit.zip`를 내보내서 원래 서명된 NVIDIA 설치 프로그램, 선택 항목, 해시 및 지침을 함께 유지합니다. `NVDriverForge.exe`를 별도로 휴대하세요. 도구에서 키트를 가져오고 미리보기를 검토한 다음 일반 설치 워크플로를 사용하세요. 이것은 슬림형 드라이버나 수정된 ​​독립 실행형 설치 프로그램이 아닙니다. 선택적 NVENC에는 여전히 정확한 드라이버에 대한 다운로드 및 동의가 필요합니다. NVIDIA의 재배포 조건은 여전히 ​​적용됩니다.

**결과 및 지원:** 짧은 결과를 읽고 단계별/옵션별 세부 정보를 확장하세요. 성공적인 다시 읽기는 측정된 개선이 아닌 저장된 값을 설정합니다. 로컬 JSON 지원 보고서는 앱을 다시 시작한 후 마지막으로 저장된 작업을 포함하여 허용 목록에 있는 필드를 사용합니다. 저장하거나 공유하기 전에 미리 보세요. 원시 로그, 프로필 콘텐츠 또는 하드웨어 식별자는 포함되지 않으며 자동으로 업로드되지 않습니다.

**복구:** 백업된 드라이버를 복구하려면 보호된 작업 가이드를 따르세요. 명시적 프로필 복원에는 원래 드라이버 버전과 동일한 GPU가 필요합니다. 전체 데이터베이스를 교체하고 현재 복사본을 보존하며 해시 및 충돌 상태를 확인합니다. 저널을 지우거나 불일치를 강요하지 마십시오. 이 새로운 워크플로우를 사용한 실제 드라이버 설치, 전체 복구 및 기본 프로필 가져오기는 실제 시스템에서 검증되지 않은 상태로 유지됩니다.

**애플리케이션 업데이트:** 릴리스 노트를 읽은 다음 SHA-256 인증 다운로드를 명시적으로 선택하세요. 확인은 기본적으로 수동으로 수행되며 시작 시 선택적으로 확인됩니다. 설치 프로그램이 자동으로 시작되지 않습니다. 이 기능은 드라이버 업데이트 확인 및 설치된 버전의 선택적 드라이버 확인 작업과 별개입니다.
...
로컬 데이터는 `%LOCALAPPDATA%\NVDriverForge` 아래에 있습니다. 보호된 작업 및 드라이버 내보내기는 `%PROGRAMDATA%\NVDriverForge\Jobs`에 있습니다. 휴대용으로 사용하면 로컬 데이터도 생성됩니다. 드라이버 저장소 내보내기와 기본 프로필 백업은 별개입니다. 시스템 이미지도 아닙니다.
...
| NVIDIA 설치 실패 | 실패 요약을 읽고 자세한 로그를 엽니다. 이미 최신이거나 적용할 수 없는 선택적 구성 요소는 0.1.4에서 건너뛸 수 있는 상태로 유지됩니다. 실패한 설치는 선택적 조정이나 성공/다시 시작 흐름을 트리거하지 않습니다. |

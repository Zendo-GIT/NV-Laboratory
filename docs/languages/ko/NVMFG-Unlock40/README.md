...
[0.2.3 및 상태 다운로드](../docs/downloads.md#nvmfg-unlock40) · [설치](#installation) · [업스트림](#upstream-and-modifications) · [라이센스](LICENSES/README.md)
...
현재 패키지는 **0.2.3**입니다. 영구 게임 라이브러리, 활동 및 기능 정보, 로컬 진단 및 수정된 선택/진행 동작을 추가합니다. [다운로드](../docs/downloads.md#nvmfg-unlock40)는 정확한 파일과 해시를 식별합니다.
...
2. 릴리스가 출시되면 `NVMFGUnlock40-0.2.3-Setup-x64.exe` 또는 `NVMFGUnlock40-0.2.3-Portable-x64.zip`를 다운로드합니다.
...
**영구 라이브러리:** 한 번의 스캔을 시작하기 전에 다양한 드라이브를 포함한 여러 게임 폴더를 선택합니다. 진행 상황이 표시되며 취소가 가능합니다. 첫 번째 스캔 후 로컬 캐시는 모든 게임 폴더를 탐색하지 않고 실행 시 라이브러리를 복원합니다. 변경사항을 찾거나 다른 폴더를 추가하려면 새로고침하세요. 유지 관리 작업에서는 영향을 받은 파일의 유효성을 다시 검사합니다. 백업 모니터링은 활성 상태로 유지됩니다. 캐시는 `%LOCALAPPDATA%\RtxMfg\library-cache.json`에 저장됩니다.

**선택:** Ctrl+A는 모두 선택하고 Ctrl+D는 활성 게임 또는 백업 탭을 지웁니다. 자동으로 선택되는 게임이 없습니다. 활동 업데이트 및 새로 고침으로 인해 더 이상 고스트 선택이나 일관되지 않은 개수가 생성되지 않습니다.

**활동 및 호환성:** 게임별 MFG 정보는 새로운 오버레이 없이 NGX 관찰에서 나온 것입니다. 표시된 프레임의 물리적 개수가 아닙니다. Dynamic-with-V-Sync 지원은 런타임 기능에서 비롯됩니다. 알 수 없는 기능은 버전 번호에서 추론되지 않습니다. 애플리케이션은 V-Sync나 VRR를 변경하지 않습니다. V-Sync를 끄면 Dynamic는 일시 중지된 상태로 유지됩니다. 고정 또는 게임 제어 선택은 별개입니다.

**다음 출시:** 임시 제외는 다음 게임 출시 시 패치를 건너뛰고 종료 후 일반 관리를 복원합니다. 게임에 이미 로드된 DLL은 제거할 수 없습니다. 해당 게임을 닫고 다시 시작하세요. Wallpaper Engine는 데스크탑 애플리케이션으로 인식됩니다. 이 수정을 통해 실제 무시된 게임에 대한 보호가 유지됩니다.

**기본 설정 및 지원:** 기본 설정 가져오기/내보내기에는 게임 폴더를 수동으로 다시 연결해야 합니다. 정보의 로컬 진단은 개인 정보를 필터링하고 사용 가능한 NVAPI 오류 코드 또는 충돌 범주를 보고합니다. 공유하기 전에 검토하세요. 아무것도 자동으로 업로드되지 않습니다.

**애플리케이션 업데이트:** 선택적 확인을 통해 릴리스 노트가 표시되고 공식 설정이 제공됩니다. 명시적 다운로드는 GitHub 크기 및 SHA-256 메타데이터를 기준으로 확인됩니다. 설치를 직접 시작합니다. 버전 0.2.3에서는 의미 있는 오류와 결과를 보존하면서 완료된 진행 메시지도 지웁니다. 이러한 추가 사항에는 공개 버전 0.1.1 이후의 변경 사항이 포함됩니다.
...
- 보고된 0.1.1 활성화/복원/제거 차단은 여전히 재현되지 않으며 그 원인을 알 수 없습니다. 이 릴리스에서는 이를 수정한다고 주장하지 않습니다. 실패 후 복구 저널을 보존하고 로컬 진단을 검사합니다. 복구 데이터를 강제로 삭제하지 마십시오.

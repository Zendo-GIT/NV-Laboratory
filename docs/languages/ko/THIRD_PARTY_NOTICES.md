...
최초 감사: 2026-09-09; 현재 패키지 및 출처 검토: 2026-09-18. NVDriverForge 0.1.4, NVMFG Unlock40 0.2.3 및 NVRasterPulse 0.2는 기존 구성 요소 라이센스를 유지합니다. Profile Inspector 3.0.2.3/setup-r2는 변경되지 않았습니다. **구성 요소의 라이선스는 전체 제품군이 아닌 해당 구성 요소에 적용됩니다.** 애플리케이션 소스는 비공개로 유지됩니다. 저작권 및 허가 고지는 그대로 유지됩니다. 이 테이블은 인덱스이지 대체 테이블이 아닙니다. "외부"는 애플리케이션 자산에 배포되지 않음을 의미합니다.

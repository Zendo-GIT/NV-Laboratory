...
[下载 0.1.4 和状态](../docs/downloads.md#nvdriverforge) · [安装](#installation) · [制作人员](#credits-and-upstream) · [许可证](../../../../NVDriverForge/LICENSE)
...
- 具有依赖关系的组件选择和未知组件的保存。
- 版本 0.1.4 使选定的可选 NVIDIA 组件保持可跳过状态，并仅从发现中排除经过验证的未检查组件。现有的或不适用的可选运行时不再被强制作为关键组件。
- 清晰的安装失败摘要并可访问所有 34 种语言的详细日志。
- 安装前进行准备情况检查、明确确认、驱动程序存储导出和本机 NVIDIA 配置文件备份。
...
- 组件指南、可重复使用的首选项、驱动程序套件、本地支持报告和可选的应用程序更新。
- 34 种界面语言和四种主题。
...
**安装之前：** 准备情况检查涵盖包签名、GPU、估计工作区/备份空间、待重启和竞争安装程序。高级工人重复这些话。竞争进程永远不会自动停止。在 NVIDIA 安装程序开始之前，本机 NVIDIA 配置文件数据库备份必须成功；驱动程序存储导出是一个单独的备份。

**可重复使用的选择：** 组件指南询问了有关游戏、音频、NVIDIA App 和录音的四个问题。审查其建议；必需的、未知的和依赖的组件仍然受到保护。导出首选项，然后在导入时预览并根据所选包重新验证它们。不会导入同意、重新启动操作、程序路径和补丁有效负载。

**驱动程序套件：** 导出 `.nvdfkit.zip`，以将原始签名的 NVIDIA 安装程序、选择、哈希值和说明保存在一起。单独携带 `NVDriverForge.exe`。在“工具”中导入套件，查看预览，然后使用正常的安装工作流程。这不是精简驱动程序或修改后的独立安装程序。可选的 NVENC 仍需要下载并同意该驱动程序。 NVIDIA 的再分发条款仍然适用。

**结果和支持：**阅读简短结果并展开每个阶段/每个选项的详细信息。成功的回读建立了一个存储值，而不是测量的改进。本地 JSON 支持报告使用白名单字段，包括重新启动应用程序后上次保存的作业。在保存或共享之前预览它。它不包含原始日志、配置文件内容或硬件标识符，并且永远不会自动上传。

**恢复：** 按照受保护作业的指南恢复备份的驱动程序。显式配置文件恢复需要原始驱动程序版本和相同的 GPU；它替换整个数据库，保留当前副本，并检查哈希值和冲突状态。不要擦除其日志或强制不匹配。使用这个新工作流程的真实驱动程序安装、完整恢复和本机配置文件导入在真实系统上仍然未经验证。

**应用程序更新：**阅读发行说明，然后明确选择经过 SHA-256 验证的下载。默认情况下，检查是手动的，可以在启动时进行可选检查。没有安装程序自动启动。此功能独立于驱动程序更新检查和已安装版本的可选驱动程序检查任务。
...
本地数据在`%LOCALAPPDATA%\NVDriverForge`下；受保护的作业和驱动程序导出位于 `%PROGRAMDATA%\NVDriverForge\Jobs` 下。便携式使用还会创建本地数据。驱动程序存储区导出和本机配置文件备份是分开的。两者都不是系统映像。
...
| NVIDIA安装失败 | 阅读故障摘要并打开详细日志。 0.1.4 中现有或不适用的可选组件仍可跳过。失败的安装不会触发可选的调整或成功/重新启动流程。 |
